...
class PostgresExporter(BaseExporter):
    ...
    def _create_engine(self):
        ...
        try:
            ...
            logger.debug(f"Creating database engine with connection string: {self._get_sanitized_connection_string()}")
    ...
    def _get_connection_string(self) -> str:
        ...
        if not all([database, user, password]):
            raise ExporterException("Missing required database connection parameters (database, user, password)")
    ...
    def export(self, data: pd.DataFrame) -> bool:
        ...
        if_exists = self.db_config.get("if_exists", "replace")  # Options: fail, replace, append
        
        try:
            logger.info(f"Exporting {len(data)} records to PostgreSQL table {schema}.{table_name}")
            ...
            logger.info(f"Successfully exported data to PostgreSQL table {schema}.{table_name}")
            return True
    ...
    def _ensure_table_schema(self, data: pd.DataFrame, table_name: str, schema: str) -> None:
        ...
        try:
            ...
            for column_name, dtype in data.dtypes.items():
                ...
                if dtype == "object":
                    ...
                    columns.append(Column(column_name, String(255)))  # Default to string
